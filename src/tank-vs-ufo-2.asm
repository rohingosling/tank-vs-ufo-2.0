//********************************************************************************
// Program: Tank vs UFO (Commodore VIC-20) - Assembly Language Rewrite
...
//================================================================================
// Zero-page variables (BASIC's zero page is free):
//
// The program never returns to BASIC, and the KERNAL IRQ touches only $90-$FF
// system areas.
//
//================================================================================
...
// Input:
//
// PEEK(197) semantics. One key at a time, no control while burning.
//
//================================================================================
...
// Quit (Q):
//
// Silence the VIC, copy a wipe stub into the cassette buffer, and run it there.
// The stub scrubs the game's RAM ($1000-$1FFF including the screen, plus zero
// page, stack and the $0200 page) and then jumps through the hardware reset
// vector for a full cold start back to BASIC.
//
//--------------------------------------------------------------------------------
...
// Bullet:
//
// One cell up per call; erases its old cell (the muzzle flash on the first step)
// and checks UFO and bomb collisions. Drawing happens in the draw phase.
//
//================================================================================
...
// Bullet hit:
//
// The UFO begins its crash dive IMMEDIATELY, while the air explosion animates
// independently at the point of contact.
//
//--------------------------------------------------------------------------------
...
// UFO flight:
//
// One cell of horizontal travel per game tick, plus the aimed bomb-drop check.
// Spawns a fresh UFO whenever the slot is empty.
//
//================================================================================
...
// Bomb:
//
// One diagonal cell per game tick. Checks the tank silhouette, the ground, and
// the bullet, in that order, like the original PEEK chain.
//
//================================================================================
...
// Ground puff:
//
// A bomb that misses bursts where it lands. Three animated frames, 150 ms each,
// all black. The dust dots sit one blank cell apart, spanning five columns
// (burst_col - 2 .. burst_col + 2):
//
//   Frame 1:    (blank)
//                \|/
//
...
// Erase the puff's full footprint, edge-clamped:
//
// The 3-cell splash row plus the three spaced dust dots above
// it (5 columns, every second cell).
...
// Tank hit:
//
// Enter the burning state. UFOs keep strafing (non-blocking) but hold their bombs
// until the new tank spawns.
//
//================================================================================
...
// Air blast:
//
// An independent three-frame animation (150 ms per frame) fixed at the bullet's
// point of contact, graded by heat:
//
//   Frame 1:   ...
//              .X.
//              ...
...
// Crash dive:
//
// The stricken UFO falls one row per step, drifting with its direction of travel,
// until it lands on the impact row and catches fire.
//
//================================================================================
...
// Flame animation:
//
// One shared phase flips the burning-tank and crashed-UFO flame glyphs between
// the original's two frames. The draw phase reads the phase every frame; this
// just advances it.
//
//================================================================================
...
// Burning tank:
//
// Flame glyphs over the gun row, solid fire over the hull row, four cells wide
// over the hull (cols tank_col+1..+4).
// The original's line 701 does A=A+1 before its TAB(A) reverse-print pattern.
// The base of the fire burns hotter (yellow) than the flame tips above it (red).
...
// Draw phase:
//
// Repaint every active entity once per frame, in a fixed layer order
...
// Sound:
//
// One mixer pass per frame. The noise voice carries the loudest active source
// (blast > fires > shot); voice 3 warbles during the dive.
//
//================================================================================
...
// Tank:
//
// The original three-row PRINT shape, drawn in blue (difference 5).
//
...
// The treads row alone, also drawn while the tank burns (the original kept
// the treads visible under the flames).
...
// UFO:
//
// "<*>" at (ufo_row, ufo_col), black like the original.
//
//--------------------------------------------------------------------------------
...
// Score row:
//
// "ufos N tanks N" in blue (difference 5; the original printed it in yellow),
// with BASIC's leading-space number formatting.
//
//--------------------------------------------------------------------------------
...
// Random numbers:
//
// 8-bit LFSR stirred with the free-running IRQ timer.
//
//================================================================================
